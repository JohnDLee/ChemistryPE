...
# Last Modified: Monday, 24th July 2023 10:27:30 am
...
def create_prompt(test, examples):
    ...
    prompt = """You are an expert chemist. Your task is to predict the next intermediate molecule in the electron transfer process of a Linear Electron Flow reaction given the reactants and reagents, several examples of reactions with intermediate steps, and your experienced reaction prediction knowledge. Additionally, there are some rules to follow.
...
if __name__ == '__main__':
    ...
    # storage dir 
    results_dir = Path(os.environ['RESULTS_DIR']) / "step2_prediction_specific" / model.value
    ...
    np.save(str(results_dir / "step2_prediction_specific_results.npy"), dict(info), allow_pickle=True)
