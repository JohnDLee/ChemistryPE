...
# Last Modified: Friday, 21st July 2023 2:01:19 pm
...
def create_prompt(test, examples):
    ...
    prompt = """You are an expert chemist. Your task is to predict the next intermediate molecules in the electron transfer process given the reactants and reagents, several examples of reactions with intermediate steps, and your experienced reaction prediction knowledge. There are some rules to follow.
...
if __name__ == '__main__':
    ...
    # storage dir 
    results_dir = Path(os.environ['RESULTS_DIR']) / "step2_prediction_full" / model.value
    ...
    np.save(str(results_dir / "step2_prediction_full_results.npy"), dict(info), allow_pickle=True)
