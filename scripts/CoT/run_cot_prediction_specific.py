...
# Last Modified: Monday, 24th July 2023 1:42:25 pm
...
def create_prompt(test, examples):
    ...
    prompt = """You are an expert chemist. Your task is to predict the resulting product in this Linear Electron Flow reaction given the reactants and reagents, several examples including intermediate steps, and your experienced reaction prediction knowledge. There are some rules to follow.
...
if __name__ == '__main__':
    ...
    # storage dir 
    results_dir = Path(os.environ['RESULTS_DIR']) / "cot_prediction_specific" / model.value
    ...
    np.save(str(results_dir / "cot_prediction_specific_results.npy"), dict(info), allow_pickle=True)
